defmodule Day11Part02 do
  @moduledoc """
  Parte 02 do dia 11 do Advent of Code 2023
  """
  ...
  def expand_rows(grid, current_row, number_of_rows, number_of_columns) do
    ...
    if row == [] do
      new_grid =
        grid
        |> Enum.map(fn {{r, c}, v} ->
          if r >= current_row, do: {{r + 999_999, c}, v}, else: {{r, c}, v}
        end)

      expand_rows(new_grid, current_row + 1_000_000, number_of_rows + 999_999, number_of_columns)
    else
  ...
  def expand_columns(grid, current_column, number_of_columns, number_of_rows) do
    ...
    if column == [] do
      new_grid =
        grid
        |> Enum.map(fn {{r, c}, v} ->
          if c >= current_column, do: {{r, c + 999_999}, v}, else: {{r, c}, v}
        end)

      expand_columns(
        new_grid,
        current_column + 1_000_000,
        number_of_columns + 999_999,
        number_of_rows
      )
    else
